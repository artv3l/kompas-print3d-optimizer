...
ksEntityPtr cutExtrusion(ksPartPtr part, ksEntityPtr sketchEntity, bool normalDirection, DocumentData::Settings& settings, int multiplier) {
	...
	{ // Привязываем размеры к переменным
		...
		oss << multiplier << " * ("
			<< settings.getNumericSetting(SI_BRIDGE_HOLE_BUILD_LAYERS_COUNT.variableName)->getExpression() << " * "
			<< settings.getNumericSetting(SI_LAYER_HEIGHT.variableName)->getExpression()
			<< ")";
...
ksEntityPtr fillBridgeHoles(KompasObjectPtr kompas, ksPartPtr part, std::list<BridgeHoleFillTarget> bridgeHoleFillTargets, DocumentData::Settings& settings) {
	...
	for (BridgeHoleFillTarget target : bridgeHoleFillTargets) {
		...
		{ // Привязываем размеры к переменным
			...
			std::ostringstream oss;
			oss << settings.getNumericSetting(SI_BRIDGE_HOLE_FILL_LAYERS_COUNT.variableName)->getExpression() << " * " << settings.getNumericSetting(SI_LAYER_HEIGHT.variableName)->getExpression();
			variable->Expression = oss.str().c_str();
		}
