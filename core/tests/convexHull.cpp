...
TEST(core_convexHull, corner_case_1)
{
    ...
    EXPECT_TRUE(convexHull(points1) == empty);
    const std::vector<Eigen::Vector2d> points2{ Eigen::Vector2d(1, 2), Eigen::Vector2d(1, 2) };
    EXPECT_TRUE(convexHull(points2) == empty);
    const std::vector<Eigen::Vector2d> points3{ Eigen::Vector2d(1, 1), Eigen::Vector2d(2, 2), Eigen::Vector2d(3, 3), Eigen::Vector2d(4, 4) };
    EXPECT_TRUE(convexHull(points3) == empty);
}
